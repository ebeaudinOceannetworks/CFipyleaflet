...
def parse_cor_header(filepath):
    ...
    with open(filepath, "r", errors="ignore") as f:
        for line in f:
            ...
            # new Station name finder
            elif line.startswith("Station name"):
                name = line.replace("Station name:", "").strip()
            
                # CF code always present
                m = re.match(r"(CF\d+)\s*(.*)?$", name)
                if m:
                    ...
                    if raw_alias:
                        ...
                        if raw_alias.startswith("(") and raw_alias.endswith(")"):
                            raw_alias = raw_alias[1:-1]
            
                        info["stationAlias"] = raw_alias if raw_alias else None
                    else:
                        info["stationAlias"] = None
...
def extract_community_from_citation(citation):
    ...
    # ---- Normal filtering ----
    community_orgs = [o for o in orgs if o not in EXCLUDE]
